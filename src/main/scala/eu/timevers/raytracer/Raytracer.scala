...
import eu.timevers.raytracer.primitives._
...
class Raytracer[F[_]](using S: Applicative[F], L: Logger[F]):
  def render(config: Config): F[Image] =
    ...
    val coloredPixels = pixels.toList.traverse { case (i, j) =>
      val message = if i == 0 then L.info(s"Scanlines remaining: $j") else S.pure(())
      message *> S.pure(generatePixel(i, j, width, height, config.cameraSettings))
    }
    coloredPixels.map(cs => Image(width, height, cs.map(RGBColor(_))))

  private def generatePixel(i: Int, j: Int, imageWidth: Int, imageHeight: Int, cameraSettings: CameraSettings): Color =
